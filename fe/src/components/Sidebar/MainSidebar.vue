                ...
                <li v-for="subItem in item.submenu" :key="subItem.name">
                  ...
                  <ul v-if="subItem.subSubMenu && expandedSubMenus[`${item.name}-${subItem.name}`]" class="ml-4">
                    <li v-for="subSubItem in subItem.subSubMenu" :key="subSubItem.name">
                      <a @click="subSubItem.subSubSubMenu ?
                        toggleSubSubMenu(`${item.name}-${subItem.name}-${subSubItem.name}`, subSubItem.path) :
                        navigateTo(subSubItem.path || '')">
                        <span>
                          {{ subSubItem.name }}
                        </span>
                        <span v-if="subSubItem.subSubSubMenu" class="flex items-center">
                          <component class="h-3 w-3"
                            :is="expandedSubSubMenus[`${item.name}-${subItem.name}-${subSubItem.name}`] ? ChevronDownIcon : ChevronRightIcon" />
                        </span>
                      </a>

                      <ul
                        v-if="subSubItem.subSubSubMenu && expandedSubSubMenus[`${item.name}-${subItem.name}-${subSubItem.name}`]"
                        class="ml-4">
                        <li v-for="subSubSubItem in subSubItem.subSubSubMenu" :key="subSubSubItem.name">
                          <a @click="navigateTo(subSubSubItem.path ?? '')" :class="[
                            activePath === subSubSubItem.path ? 'bg-base-300 text-base-content' : '',
                            { 'font-bold': activePath === subSubSubItem.path }
                          ]">
                            {{ subSubSubItem.name }}
                          </a>
                        </li>
    ...
    <div v-if="isCollapsed && hoveredMenuDetails && hoveredMenu" ref="hoverMenuRef"
      ...
      <ul class="menu menu-md p-2">
        <li v-for="subItem in hoveredMenuDetails.submenu" :key="subItem.name">
          <a @click="subItem.subSubMenu ?
            toggleSubMenu(`${hoveredMenuDetails.name}-${subItem.name}`, subItem.path) :
            navigateTo(subItem.path || '')"
            :class="[subItem.isActive ? 'bg-secondary text-secondary-content my-1' : '']">
            <span :class="{ 'font-bold': subItem.isActive }">
              {{ subItem.name }}
            </span>
            <span v-if="subItem.subSubMenu" class="flex items-center">
          ...
          <ul v-if="subItem.subSubMenu && expandedSubMenus[`${hoveredMenuDetails.name}-${subItem.name}`]" class="ml-4">
            <li v-for="subSubItem in subItem.subSubMenu" :key="subSubItem.name">
              <a @click="subSubItem.subSubSubMenu ?
                toggleSubSubMenu(`${hoveredMenuDetails.name}-${subItem.name}-${subSubItem.name}`, subSubItem.path) :
                navigateTo(subSubItem.path || '')"
                :class="[activePath === subSubItem.path ? 'bg-base-300 text-base-content my-1' : '']">
                <span :class="{ 'font-bold': activePath === subSubItem.path }">
                  {{ subSubItem.name }}
                </span>
                <span v-if="subSubItem.subSubSubMenu" class="flex items-center">
                  <component class="h-3 w-3"
                    :is="expandedSubSubMenus[`${hoveredMenuDetails.name}-${subItem.name}-${subSubItem.name}`] ? ChevronDownIcon : ChevronRightIcon" />
                </span>
              </a>

              <ul
                v-if="subSubItem.subSubSubMenu && expandedSubSubMenus[`${hoveredMenuDetails.name}-${subItem.name}-${subSubItem.name}`]"
                class="ml-4">
                <li v-for="subSubSubItem in subSubItem.subSubSubMenu" :key="subSubSubItem.name">
                  <a @click="navigateTo(subSubSubItem.path ?? '')" :class="[
                    activePath === subSubSubItem.path ? 'bg-base-300 text-base-content' : '',
...
const expandedSubSubMenus = ref<{ [key: string]: boolean }>({});
...
// Fungsi baru untuk toggle level 4
const toggleSubSubMenu = (subSubMenuKey: string, path?: string): void => {
  expandedSubSubMenus.value[subSubMenuKey] = !expandedSubSubMenus.value[subSubMenuKey];
  if (path) {
    navigateTo(path);
  }
};
